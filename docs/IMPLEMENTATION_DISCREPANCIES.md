...
**Global / Cross‑System**
- Namespaces in existing core systems are `Core` while new systems are `HustleEconomy.Core`. This is inconsistent with the Master Implementation Guide.
...
- Several systems referenced in specs are missing: `EventSystem`, `InventorySystem`, `ActivitySystem`, `CriminalRecordSystem`, `CameraEffects`, `MinigameSystem`, `VehicleSystem`, `ClothingSystem`.
...
- Added local `ActivityType` enum because ActivitySystem is missing.
...
- Auto‑generation uses `RelationshipSystem.CreateNPC` to obtain name; spec expects `GetNPC`.
